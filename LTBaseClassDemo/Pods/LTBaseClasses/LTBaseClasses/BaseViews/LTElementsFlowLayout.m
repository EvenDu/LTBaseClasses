...
@interface LTElementsFlowLayout ()

/** 所有的cell的attrbts */
@property (nonatomic, strong) NSMutableArray<UICollectionViewLayoutAttributes *> *lt_AtrbsArray;

/** 每一列的最后的高度 */
/** <#digest#> */
@property (assign, nonatomic) CGRect lt_LastAtrbsFrame;
...
@implementation LTElementsFlowLayout
...
- (void)prepareLayout
{
    ...
    //如果重新刷新就需要移除之前存储的高度
    //复赋值以顶部的高度, 并且根据列数
    self.lt_LastAtrbsFrame = CGRectMake(0, 0, self.collectionView.frame.size.width, 0);
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGFloat rightLeftWidth = self.collectionView.frame.size.width - CGRectGetMaxX(self.lt_LastAtrbsFrame) - [self xMarginAtIndexPath:indexPath] - self.edgeInsets.right;
    ...
    if (rightLeftWidth >= w) {
        ...
        x = CGRectGetMaxX(self.lt_LastAtrbsFrame) + [self xMarginAtIndexPath:indexPath];
        y = self.lt_LastAtrbsFrame.origin.y;
    ...
    // 覆盖添加完后那一列;的最新高度
    self.lt_LastAtrbsFrame = atrbs.frame;
...
- (CGRect)maxHeightFrame
{
    __block CGRect maxHeightFrame = self.lt_LastAtrbsFrame;
    
    
    [self.lt_AtrbsArray enumerateObjectsUsingBlock:^(UICollectionViewLayoutAttributes * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (id<LTElementsFlowLayoutDelegate>)delegate
{
    return (id<LTElementsFlowLayoutDelegate>)self.collectionView.dataSource;
}

- (instancetype)initWithDelegate:(id<LTElementsFlowLayoutDelegate>)delegate
{
    if (self = [super init]) {
...
+ (instancetype)flowLayoutWithDelegate:(id<LTElementsFlowLayoutDelegate>)delegate
{
